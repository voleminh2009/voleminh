...
var (
	...
	headersUseragents []string = []string{
		...
		"Mozilla/5.0 (Macintosh; IntelMozilla/5.0 (iPhone; CPU iPhone OS 8_1_2 like Mac OS X) AppleWebKit/600.1.4 (KHTML, like Gecko) Version/8.0 Mobile/12B440 Safari/600.1.4",
...
func main() {
	...
	go func() {
		fmt.Println("-- ｖｌｍ　（ホタス） Attack Started --\n           Go!\n\n")
		ss := make(chan uint8, 8)
		var (
			err, sent int32
		)
		fmt.Println("𝓘𝓷 𝓾𝓼𝓮               |\t𝐑𝐞𝐬𝐩 𝐎𝐊 |\t𝙂𝙤𝙩 𝙚𝙧𝙧")
		for {
			...
			case targetComplete:
				sent++
				fmt.Printf("\r%-6d of max %-6d |\t%7d |\t%6d", cur, maxproc, sent, err)
				fmt.Println("\r-- ｖｌｍ　（ホタス） Attack Finished --       \n\n\r")
				os.Exit(0)
			}
		}
	}()
